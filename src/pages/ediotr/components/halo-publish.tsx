...
export default function HaloPublish(props: Props) {
    ...
    const {theme, dark_theme} = useSelector(
    ...
    return (
        <Form
            form={form}
            onSubmit={async (val) => {
                let html = html_content + `<style>${theme}${dark_theme}</style>`;
                val.content = html;
                val.originalContent = html;
                if (article.halo_id) {
        ...
        >
            ...
            <Form.Item label="分类" field="categoryIds">
                <TreeSelect showSearch allowClear treeCheckable treeData={categories}/>
            </Form.Item>
