...
import { getThemeDetail, getThemeList, getThemePublic } from "@/api/theme";
...
export default function Themes() {
  ...
  const [themeID, setThemeID] = useState(
    localStorage.getItem("theme_id") || "999"
  );
  //我的主题
  ...
  return (
    <>
      <div className={"theme-title"}>公开主题</div>
        ...
        {theme.map((item) => {
          return (
            <MenusItem
              key={item.id}
              value={item.id}
              auth={item.user?.nick_name}
              radio={true}
              title={item.name}
            />
          );
        })}
        <div className={"theme-title"}>我的主题</div>
        {myTheme?.map((item) => {
          return (
            <MenusItem
              key={item.id}
