  ...
  const QiNiuComponent = () => {
    return (
      <>
        <FormItem
          label="AccessKey"
          field="qi_niu_access_key"
          rules={[{ required: true, message: "AccessKey必填" }]}
        >
          <Input.Password />
        </FormItem>
        <FormItem
          label="SecretKey"
          field="qi_niu_secret_key"
          rules={[{ required: true, message: "SecretKey必填" }]}
        >
          <Input.Password />
        </FormItem>
        <FormItem
          label="空间名称"
          field="qi_niu_bucket"
          rules={[{ required: true, message: "Bucket必填" }]}
        >
          <Input />
        </FormItem>
        <FormItem
          label="绑定域名"
          field="qi_niu_domain"
          rules={[{ required: true, message: "Domain必填" }]}
        >
          <Input />
        </FormItem>
    ...
    return (
      <>
        ...
        <FormItem
          label="操作员授权码"
          field="up_yun_pass"
          rules={[{ required: true, message: "PassWord必填" }]}
        >
          <Input.Password />
        </FormItem>
        <FormItem label="域名是否是Https" field="is_https" initialValue={false}>
    ...
    return (
      <>
        <FormItem
          label="Token"
          field="ski_token"
          rules={[{ required: true, message: "Token必填" }]}
        >
    ...
    <div>
      <Form onSubmit={onSubmitFunc} form={form}>
        <FormItem label="选择云存储" field="oss_type">
          ...
          >
            <Option key="qiniu" value="qiniu">
              七牛云
            </Option>
            <Option key="youpai" value="youpai">
              又拍云
            </Option>
