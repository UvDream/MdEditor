...
import {deleteFile__openAPI__deleteId, getFileList} from "@/api/file";
import config from "@/config";
...
export default function ImgList(props: Props) {
    ...
    const deleteImg = async (id: string) => {
        //@ts-ignore
        const res = (await deleteFile__openAPI__deleteId({
            id,
        })) as unknown as API.Response;
        if (res.code === 200) {
                                ...
                                <Col key={index} span={6}>
                                        ...
                                        cover={
                                            <div style={{height: 180, overflow: "hidden"}}>
                                                <Image
                                                    width={"100%"}
                                                    src={
                                                        (item.is_https ? "https://" : "http://") + (item.position == "local"
                                    ...
                                    >
                                        <Meta
                                            ...
                                            description={
                                                <Tooltip content={item.create_time}>
